...
chrome.storage.local.get("jwt", ({ jwt }) => {
  if (!jwt) {
    window.location.href = chrome.runtime.getURL("auth.html?redirect=report.html");
    return;
  }

  // Proceed with chart logic
  chrome.storage.local.get("scrapedReviews", (data) => {
    ...
    new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Total Reviews', data: totalReviews, borderWidth: 2 },
          { label: 'Average Rating', data: avgRatings, borderWidth: 2 }
        ]
      }
    });
  });
